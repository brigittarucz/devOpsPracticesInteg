...
module.exports = class User {
  ...
  createUser(database) {
    return database.execute(
      'INSERT INTO users (id, email, password, proffesion, experience, interests, events) VALUES (?, ?, ?, ?, ?, ?, ?)',
  ...
  saveUser() {
    return db.execute(
      'UPDATE users SET email = ?, password = ?, proffesion = ?, experience = ?, interests = ? WHERE users.id = ?',
