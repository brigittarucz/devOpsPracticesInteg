...
exports.getProfile = (req, res) => {
  ...
  if (req.headers.test !== 'true') {
    User.fetchUserById(localStorage.getItem('sessionId'), db)
      .then(aUser => {
        var user = aUser[0][0];
...
exports.postAddFromSuggested = (req, res) => {
  if (localStorage.getItem('sessionId') == null) {
    return res.render('auth/authenticate', {
      pageTitle: 'Authentication',
    });
    ...
    .then(user => {
      ...
      if (sUserExists) {
        ...
        utilities.addToEventArray(user, req).then(result => {
          ...
          User.updateUserEvents(localStorage.getItem('sessionId'), sUpdatedEvents)
            .then(() => {
              return res.redirect('/profile');
            })
            .catch(error => {
...
exports.postDeleteFromCalendar = (req, res) => {
  // if(localStorage.getItem('sessionId') == null) {
  //     return res.render('auth/authenticate', {
  //         pageTitle: 'Authentication',
  //     })
  ...
  User.fetchUserById(localStorage.getItem('sessionId'), db)
    .then(user => {
      ...
      User.updateUserEvents(localStorage.getItem('sessionId'), aUserEvents.length > 0 ? JSON.stringify(aUserEvents) : '')
        .then(() => {
          return res.redirect('/profile');
        })
        .catch(error => {
